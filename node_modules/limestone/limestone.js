...
exports.SphinxClient = function() {
    ...
    function _enqueue(req_buf , cb, sc) {
        if(!server_conn || !server_conn.writable){
            console.error("Trying to enqueue. Not connected");
            cb(Sphinx.error.DISCONNECT);
    ...
    function readResponseData() { // Got response!
        var data = this.read();
        if(!response_output){ //add by jee
            console.error('limestone fail recv data ' + data);
            return;
        }
        ...
        response_output = {
            ...
            parseHeader : function() {
                if (this.status === null && this.data.length >= 8) {
                    ...
                    //console.error(decoder.string(this.data.length));
            ...
            checkResponse : function(search_command) {
                ...
                if (this.version < search_command) {
                    errmsg += util.format("Searchd command(%s) older than client's version(%s), some options might not work", this.version, search_command);
                }
    ...
    var parseSearchResponse = function (data) {
        ...
        for (i = 0; i < output.match_count; i++) {
            ...
            if (output.id64 == 1) {
                ...
                match.doc = id64[0]*Math.pow(2,32) + id64[1];
